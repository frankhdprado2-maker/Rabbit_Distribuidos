...
from src.cuentas_cobrar.service import procesar_cuenta
from src.shared.constants import QUEUE_CXC, SERVICE_CXC
...
logger = configure_logging(SERVICE_CXC)
...
def callback(ch: Any, method: Any, properties: Any, body: bytes) -> None:
    try:
        ...
        accion, next_event = procesar_cuenta(event, ch)
        ...
        logger.exception("Error detectado en cuentas por cobrar: %s", exc)
        ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)


def main() -> None:
    logger.info("Servicio iniciado. Cola consumida: %s", QUEUE_CXC)
    connection = get_rabbit_connection()
    channel = connection.channel()
    declare_topology(channel)
    channel.basic_qos(prefetch_count=1)
    channel.basic_consume(queue=QUEUE_CXC, on_message_callback=callback, auto_ack=False)
